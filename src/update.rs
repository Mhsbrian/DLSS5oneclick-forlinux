...
pub fn download_and_swap(av: &Available, progress: &(dyn Fn(u8, &str) + Sync)) -> Result<PathBuf> {
    ...
    // and over, each start offering the same update again (#26). An AppImage
    // compresses the binary inside a squashfs, hiding the string — the check
    // only applies to the plain binaries.
    if appimage().is_none() && !carries_version(&head, &av.version) {
        let _ = std::fs::remove_file(&fresh);
        bail!(
            "the {} download does not identify itself as {}: the release asset is not the \
             version its tag claims. Nothing was changed; please report it on the issue tracker.",
            av.tag,
            av.version
        );
    }
