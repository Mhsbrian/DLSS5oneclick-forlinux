...
/// Where a downloaded file is kept for next time. On Linux this follows the XDG
/// cache convention (`~/.cache/dlss5oneclick/downloads`), the same base the
/// poster cache uses; on Windows it is `%LOCALAPPDATA%\dlss5oneclick\downloads`.
pub fn cache_dir() -> std::path::PathBuf {
    ...
    let base = std::env::var_os("LOCALAPPDATA")
        .map(std::path::PathBuf::from)
        .unwrap_or_else(std::env::temp_dir);
    base.join("dlss5oneclick").join("downloads")
}
