...
Both come from the same build and update themselves from this repo. The AppImage needs no libfuse2 (it uses the static type2 runtime; where FUSE is missing entirely, run it with `APPIMAGE_EXTRACT_AND_RUN=1`), carries desktop and AppStream metadata, and works with AppImage managers such as Gear Lever or AppImageUpdate, which fetch updates as deltas through the `.zsync` published beside it. Every release lists `SHA256SUMS` — `sha256sum -c --ignore-missing SHA256SUMS` in your download folder checks what you got — and `--version` prints the version you are running.
...
- **Game library built in.** Your installed Steam games (native, Flatpak or Snap Steam, all library folders, and non-Steam games you added to Steam), Heroic (Epic/GOG) and Lutris games are listed in the app — click one instead of hunting for the folder. `--list-games` does the same in the terminal, and the CLI takes a game name or Steam appid: `dlss5oneclick "skyrim special"`.
- **Launch options handled.** ReShade/OptiScaler load as `dxgi.dll`, which Proton only picks up with `WINEDLLOVERRIDES="dxgi=n,b" %command%` in the game's launch options. After an install the tool sets this **for you** (Steam closed: it edits `localconfig.vdf` — or `shortcuts.vdf` for a non-Steam game — surgically, with backups; Steam running: it shows the exact string with a copy button). Existing launch options are merged, never overwritten — your `MANGOHUD=1`, game arguments, and other DLL overrides survive. `--launch-options` / `--revert-launch-options` do it from the terminal. On old Proton (< 9) `PROTON_ENABLE_NVAPI=1` is added too; Heroic gets its per-game environment variables written (experimental), Lutris users get the exact variables to paste.
...
- **The model is checked against your card before it installs.** The 165 MB `nvngx_dlssnr.dll` is compiled for a set of CUDA architectures; the wrong build loads and then silently renders nothing. The tool opens the downloaded model, reads the `sm_XX` architectures its fatbins actually carry, reads your card's exact compute capability from `nvidia-smi`, and **refuses a build that has no code for your card** with the reason — instead of leaving you to debug a silent no-op. It also holds the `renodx-dlss5` add-on on the classic 4.55 build when your DLSS5-Feeder is too old for anything newer, or when the Feeder has measured the newer build faulting on your driver — avoiding the `CreateFeature 0xC0000005` crash.
- **Tune the frame cost, or add frame generation.** Neural rendering is not free — a moderate frame cost on RTX 40, a heavy one on 20/30. **Model resolution** buttons (`--model-res`, OptiScaler engine) run the neural model at a fraction of native — 75 % ≈ half the cost, 50 % ≈ a quarter — while the frame keeps full detail; the dial opens on what the game's `OptiScaler.ini` already says, so a reinstall keeps your choice. And **FSR 3.1 frame generation** (`--fg`, any RTX card, D3D12) switches on the frame-gen libraries OptiScaler already ships, for a 2× frame count. Both are experimental under Proton — see [Will it actually work?](#will-it-actually-work-stability-under-proton) below.
...
- **RTX 40 multi-frame generation** (optional, `--mfg` or a GUI tick). For RTX 40 cards, on games that already have DLSS Frame Generation (Cyberpunk 2077, …), it lifts the multiplier to 3X and above, up to 6X — upstream's approach: on the ReShade engine [mavismmg's MFGAdaUnlock-RenoDx](https://github.com/mavismmg/MFGAdaUnlock-RenoDx) add-on (one MIT file that patches memory only — no proxy DLL, no extra launch option) plus the newest `nvngx_dlssg.dll` it validates against (the game's own is kept and put back on Remove); on the pre-SR OptiScaler build, that build's own unlock. The tick is disabled with the reason on other cards, 32-bit games, games without Frame Generation, and the stable OptiScaler build. **Experimental under Proton.** This port used to ship dashdogy's RTX40MFG-Unlock for the same job: a game that still has it gets it taken out before the new one goes in (the two must never run together), and Remove still takes it out.
...
CLI: `dlss5oneclick <folder | name | appid>` installs · `--check` detect only · `--diagnose` · `--imports` (print the exe's import table and the API the scan reads) · `--remove` / `--remove-all` · `--engine=opti` · `--fg` (OptiScaler FSR 3.1 frame generation, any RTX card) · `--model-res=25..100` (OptiScaler neural model resolution — cost falls with the square, 75 ≈ half) · `--renodx` · `--upstream` (experimental Neural Upstream consumer) · `--mfg` (RTX 40 multi-frame generation) · `--mode=feeder|native` · `--bridge` (force the DX11 bridge when the renderer can't be proven) · `--remix-swap` (replace an RTX Remix runtime that has no neural pass) · `--remix-list` (list known RTX Remix projects, mark the ones you own) · `--install-remix-mod` / `--remove-remix-mod` (download & lay in a game's Remix mod, or take it back out) · `--ignore-anticheat` · `--launch-options` / `--revert-launch-options` · `--list-games` · `--update` · `--version`.
...
2. **Lower the model resolution** — the **Model resolution** buttons on the OptiScaler engine, or `--model-res=50`. The neural model then runs at a fraction of the pixels (a quarter at 50 %), a large drop in GPU load; the frame itself keeps full detail.
...
- `d3d9=n,b` — only on the DirectX 9 route (dgVoodoo's `d3d9.dll`) or with an RTX Remix bridge `d3d9.dll` beside the exe; without it Proton renders through its own d3d9 and the route silently does nothing.
- `dinput8=n,b` — only when `dinput8.dll` is REFramework (RE Engine games); another mod's `dinput8.dll` is left to its owner.
- `<proxy>=n,b` — only while the older dashdogy MFG unlock is still installed, for the loader DLL it placed; the current unlock needs no override.
...
**Releasing.** Bump `version` in `Cargo.toml`, commit, then tag and push the tag: `git tag v0.18.0 && git push origin v0.18.0`. The **Release** workflow (`.github/workflows/release.yml`) builds inside `ubuntu:22.04`, so the binary needs nothing newer than glibc 2.35. It refuses a tag that does not match `Cargo.toml`, runs clippy and the tests, packages the AppImage with a pinned, checksum-verified appimagetool and runtime, checks that both the binary and the AppImage report the tag's version, and publishes `dlss5oneclick-linux-x86_64`, `dlss5oneclick-x86_64.AppImage`, its `.zsync` and `SHA256SUMS` to the tag's release with generated notes. A suffixed tag (`v0.18.0-rc.1`) becomes a pre-release, which the self-updater never offers. Running the workflow by hand builds the same files as a downloadable artifact and publishes nothing. `packaging/release-build.sh` is the whole build job; its header has the `docker run` line that reproduces a release build locally.

Verified 2026-09-07 on Arch (Hyprland/Wayland, RTX 4090, driver 610.57): library discovery against a real Steam install; `--check`/`--diagnose` against Cyberpunk 2077, Bodycam and Baldur's Gate 3; OptiScaler installs that placed the model and switched neural rendering on (composition dispatches confirmed in `OptiScaler.log`); the CUDA fatbin check against the real `.SF` model on disk (reads `{sm_75, 86, 89, 120}`); the `WorkingScale`/frame-gen ini edits against a real 1500-line `OptiScaler.ini`; launch-option merging against real hand-written `localconfig.vdf` entries (idempotent); and the RTX Remix route against a synthetic `.trex` game. The **instability guidance above is not theoretical** — the same box both ran neural rendering cleanly *and* hit `VK_ERROR_DEVICE_LOST` / `Xid` GPU faults at 4K with DLSS Frame Generation on, which is what the diagnosis, the model-resolution dial, and the FG-off advice are for. 254 tests, `clippy -D warnings` clean, on ubuntu and windows CI.
